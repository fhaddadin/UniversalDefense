...
if (!in_array($layout_style, array('full','container','container-fluid'))) {
    ...
    if (($layout_style === '') || ($layout_style == '-1')) {
        $layout_style = g5plus_get_option('single_product_layout','container');
    }
}
...
if (!in_array($sidebar, array('none','left','right','both'))) {
    ...
    if (($sidebar === '') || ($sidebar == '-1')) {
        $sidebar = g5plus_get_option('single_product_sidebar','left');
    }
}
...
if (($left_sidebar === '') || ($left_sidebar == '-1')) {
    $left_sidebar = g5plus_get_option('single_product_left_sidebar','woocommerce');
}

$right_sidebar = g5plus_rwmb_meta($prefix.'page_right_sidebar');
if (($right_sidebar === '') || ($right_sidebar == '-1')) {
    $right_sidebar = g5plus_get_option('single_product_right_sidebar','woocommerce');
}
...
if (!in_array($sidebar_width, array('small','large'))) {
    ...
    if (($sidebar_width === '') || ($sidebar_width == '-1')) {
        $sidebar_width = g5plus_get_option('single_product_sidebar_width','small');
    }
}
...
<main  class="<?php echo join(' ',$main_class) ?>">
    ...
    <?php endif;?>
        ...
        <?php endif;?>
<div class="meta-acf">
 <h3 style="border-bottom: 1px solid #e5c217 ;padding-botton:5px;">Product Specifications</h3> 
<div class="acf-data"><pre> <?php the_field('product_specification', get_the_ID()); ?></pre></div>
 <h3 style="border-bottom: 1px solid #e5c217 ; padding-botton:5px;">Product Specifications images</h3>
<div class="acf-images">
<?php
$image = get_field('product_specification_image');
if( $image ) {
